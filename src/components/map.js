...
// ── En Route Scene (drone flying toward incident) ─────────
// Animated drone moving along route line with countdown
let enRouteAnimation = null;

export function showEnRouteScene(droneStart, incidentCoords, searchRadius, { onProgress, onArrival } = {}) {
  if (!map) return;

  // Search zone around incident
  const zone = L.circle(incidentCoords, {
    radius: searchRadius,
    color: '#fff',
    weight: 2.5,
    opacity: 0.6,
    dashArray: '2, 10',
    lineCap: 'round',
    fillColor: MC.zoneFill,
    fillOpacity: 0.12,
  }).addTo(map);
  routeLineOverlays.push(zone);

  // Search area label
  const zoneLabel = L.marker(incidentCoords, {
    icon: L.divIcon({
      className: 'search-zone-label',
      html: 'SEARCH AREA',
      iconSize: [110, 20],
  ...
  routeLineOverlays.push(zoneLabel);

  // Route line — Level 3 Emphasis (dots)
  const casing = L.polyline([droneStart, incidentCoords], {
    color: '#000', weight: 8, opacity: 0.4, lineCap: 'round',
  }).addTo(map);
  routeLineOverlays.push(casing);
  const routeLine = L.polyline([droneStart, incidentCoords], {
    color: '#fff', weight: 4, opacity: 0.95,
    dashArray: '2, 10', lineCap: 'round',
  }).addTo(map);
  routeLineOverlays.push(routeLine);

  // Distance + ETA label at midpoint
  const R = 6371;
  const dLat = (incidentCoords[0] - droneStart[0]) * Math.PI / 180;
  const dLng = (incidentCoords[1] - droneStart[1]) * Math.PI / 180;
  const a = Math.sin(dLat / 2) ** 2 + Math.cos(droneStart[0] * Math.PI / 180) * Math.cos(incidentCoords[0] * Math.PI / 180) * Math.sin(dLng / 2) ** 2;
  const distKm = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  const totalSec = Math.round(distKm / 60 * 3600);
  const etaStr = totalSec >= 60 ? `${Math.floor(totalSec / 60)}m ${totalSec % 60}s` : `${totalSec}s`;

  const mid = [(droneStart[0] + incidentCoords[0]) / 2, (droneStart[1] + incidentCoords[1]) / 2];
  const etaLabel = L.marker(mid, {
    icon: L.divIcon({
      className: 'route-label route-label-primary',
      html: `${distKm.toFixed(1)} km · ${etaStr}`,
      iconSize: [120, 22],
  ...
  routeLineOverlays.push(etaLabel);

  // Drone heading toward incident
  const dLng2 = (incidentCoords[1] - droneStart[1]) * Math.PI / 180;
  const lat1 = droneStart[0] * Math.PI / 180;
  const lat2 = incidentCoords[0] * Math.PI / 180;
  const y = Math.sin(dLng2) * Math.cos(lat2);
  const x = Math.cos(lat1) * Math.sin(lat2) - Math.sin(lat1) * Math.cos(lat2) * Math.cos(dLng2);
  const headingDeg = ((Math.atan2(y, x) * 180 / Math.PI) + 360) % 360;

  // Drone marker
  const droneIcon = L.marker(droneStart, {
    icon: L.divIcon({
      className: 'fleet-drone-marker',
      html: `<div class="fleet-drone-dot" style="--dot-color:${MC.droneFriendly}">
        <svg viewBox="0 0 24 24" width="16" height="16" xmlns="http://www.w3.org/2000/svg" style="transform:rotate(${Math.round(headingDeg)}deg)">
          <path d="M12 4 L3 18 L6 16.5 L12 15 L18 16.5 L21 18 Z" fill="#fff" stroke="none"/>
        </svg>
      </div>`,
  ...
  routeLineOverlays.push(droneIcon);

  // Fit both points in view
  const bounds = L.latLngBounds([droneStart, incidentCoords]);
  map.fitBounds(bounds, { padding: [80, 80], maxZoom: 14 });

  // Animate drone along route
  const ANIM_DURATION = 12000; // 12 seconds total animation
  const startTime = Date.now();
  const startLat = droneStart[0];
  const startLng = droneStart[1];
  const endLat = incidentCoords[0];
  const endLng = incidentCoords[1];
  ...
  function animateFrame() {
    ...
    droneIcon.setLatLng([lat, lng]);

    // Update the route line to show remaining path
    routeLine.setLatLngs([[lat, lng], incidentCoords]);
    casing.setLatLngs([[lat, lng], incidentCoords]);

    // Update ETA label
    const remainingSec = Math.round(totalSec * (1 - eased));
    const remStr = remainingSec >= 60 ? `${Math.floor(remainingSec / 60)}m ${remainingSec % 60}s` : `${remainingSec}s`;
    const remDist = distKm * (1 - eased);
    etaLabel.setLatLng([(lat + endLat) / 2, (lng + endLng) / 2]);
    etaLabel.setIcon(L.divIcon({
      className: 'route-label route-label-primary',
      html: `${remDist.toFixed(1)} km · ${remStr}`,
      iconSize: [120, 22],
      iconAnchor: [60, 11],
    }));

    if (onProgress) onProgress(eased, remainingSec);

    if (t < 1) {
      enRouteAnimation = requestAnimationFrame(animateFrame);
    } else {
      enRouteAnimation = null;
...
export function cancelEnRouteAnimation() {
  if (enRouteAnimation) {
    cancelAnimationFrame(enRouteAnimation);
    enRouteAnimation = null;
  }
}
